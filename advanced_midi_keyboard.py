...
QTextEdit, QListWidget, QTableWidget, QTextBrowser { background-color: #252525; border: 1px solid #555; border-radius: 4px; }
...
class WarningDialog(QDialog):
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        message_label = QLabel(
            "This program is designed to interface with <strong>Roblox</strong>, "
            "specifically on servers running <strong>\"Visual Pianos\"</strong>."
            "<br><br>"
            "Functionality is not guaranteed in other games or environments."
        )
...
class TutorialDialog(QDialog):
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setWindowTitle("Welcome to the MIDI Simulator!")
        self.setMinimumSize(500, 450) # Increased height for more text
        self.setStyleSheet(DARK_STYLESHEET)
        ...
        # --- IMPORTANT --- Replace "YOUR_YOUTUBE_LINK_HERE" with your actual YouTube video URL
        youtube_link = "https://www.youtube.com/watch?v=M_bzfHJgzkg"
        
        text_browser.setHtml(f"""
            <h2 style='color: #a0a0e0;'>How to Use the Program</h2>
            <p>Here’s a quick guide to get you started:</p>
            
            <h3 style='color: #e0e0e0;'>1. Loading MIDI Files</h3>
            <p>
                Click the <strong>"Add Files"</strong> button under the "Playlist" section to load one or more 
                <code>.mid</code> or <code>.midi</code> files. You can select multiple files at once.
            </p>

            <h3 style='color: #e0e0e0;'>2. Playing a Song in Roblox</h3>
            <p>
                Once loaded, select a song from the playlist. Press the <strong>"Play"</strong> button.
                A countdown will begin (you can adjust this in the GUI). 
                <br><strong>IMPORTANT: During this countdown, you MUST switch to the Roblox window ("Visual Pianos") for the notes to play correctly.</strong>
            </p>

            <h3 style='color: #e0e0e0;'>3. Generating Text</h3>
            <p>
                This program can convert MIDI notes into text characters based on a key map.
                <ul>
                    <li>Select a song and click <strong>"Generate & View Text"</strong>.</li>
                    <li>The text will appear in the <strong>"Text Mapping"</strong> tab to be copied or saved.</li>
                </ul>
            </p>

            <h3 style='color: #e0e0e0;'>4. Using a Live MIDI Keyboard</h3>
            <p>
                You can also connect a physical MIDI keyboard.
                <ul>
                    <li>Select your device from the dropdown under <strong>"MIDI Input"</strong> and click <strong>"Connect"</strong>.</li>
                    <li>The visualizer will show notes as you play.</li>
                </ul>
            </p>
            <hr>
            <h3 style='color: #a0a0e0;'>Visual Tutorial</h3>
            <p>
                For a video guide demonstrating how to use the program, please watch this tutorial:
                <br><a href='{youtube_link}' style='color: #4cc2ff;'>Watch on YouTube</a>
            </p>
            <hr>
            <p>You can view this tutorial again via the <strong>"Show Tutorial"</strong> button.</p>
        """)
...
class MidiPlayerThread(QThread):
    ...
    def __init__(self, midi_info: dict, note_map: Dict[int, str], start_delay: int):
        super().__init__()
        self.events = midi_info["events"]
        self.note_map = note_map
        self.default_sustain_on = not midi_info["has_sustain"]
    ...
    def run(self):
        try:
            ...
            while self.is_running:
                # Use iter_pending() for non-blocking message checks
            ...
            self.log_message.emit(f"MIDI Input Error: {e}")
        finally:
            if self.port and not self.port.closed:
                self.port.close()
        
        self.active_notes.clear()
        self.note_event.emit({})
        self.log_message.emit("Stopped listening.")

    def stop(self):
        # Simply set the flag. The run loop will see it and exit.
...
class MidiKeyboardApp(QMainWindow):
    ...
    def _create_controls_group(self) -> QGroupBox:
        ...
        self.delay_spinbox = QSpinBox(); self.delay_spinbox.setRange(0, 10); self.delay_spinbox.setValue(3); self.delay_spinbox.setSuffix(" s")

        layout.addLayout(button_layout, 0, 0, 1, 2)
        layout.addWidget(self.progress_bar, 1, 0, 1, 2)
        layout.addWidget(QLabel("Start Delay:"), 2, 0)
    ...
    def _create_transform_group(self) -> QGroupBox:
        ...
        self.show_tutorial_button = QPushButton("Show Tutorial"); self.show_tutorial_button.clicked.connect(self.show_tutorial)
        button_layout.addWidget(self.edit_map_button)
        button_layout.addWidget(self.show_tutorial_button)
    ...
    def _create_input_group(self) -> QGroupBox:
        ...
        try:
            ...
            # Silently fail if mido backend isn't available on start
            pass
        ...
        layout.addWidget(self.midi_input_combo, 1) # Add stretch factor
        layout.addWidget(self.refresh_midi_button)
    ...
    def _handle_first_start(self):
        """Checks if it's the first time the app is run and shows a warning and tutorial."""
        ...
        if os.path.exists(CONFIG_FILE):
            ...
            try:
                initial_setup_complete = config.getboolean('General', 'initial_setup_complete')
        ...
        if not initial_setup_complete:
            ...
            config.set('General', 'initial_setup_complete', 'true')
            with open(CONFIG_FILE, 'w') as configfile:
                config.write(configfile)

    def show_tutorial(self):
        """Creates and shows the tutorial dialog."""
    ...
    def start_playback(self, midi_info: dict):
        ...
        start_delay = self.delay_spinbox.value()
        self.player_thread = MidiPlayerThread(midi_info, self.note_map, start_delay)
    ...
    def refresh_midi_inputs(self):
        """Clears and repopulates the MIDI input device dropdown."""
        # Do not allow refreshing while a device is actively connected
        if self.input_thread and self.input_thread.isRunning():
            self.status_label.setText("Please disconnect from the current device to refresh.")
            return

        current_selection = self.midi_input_combo.currentText()
        self.midi_input_combo.clear()
        try:
            available_devices = mido.get_input_names()
            self.midi_input_combo.addItems(available_devices)
            
            # Attempt to restore the previous selection if it still exists
            if current_selection and current_selection in available_devices:
                self.midi_input_combo.setCurrentText(current_selection)

            self.status_label.setText(f"Refreshed MIDI devices list. Found {len(available_devices)}.")
        except Exception as e:
            self.status_label.setText("Could not refresh MIDI devices.")
            self.append_to_log(f"Error refreshing MIDI inputs: {e}")
    ...
    def toggle_midi_input(self, checked: bool):
        if checked:
            ...
            self.input_thread.start(); self.connect_button.setText("Disconnect"); self.midi_input_combo.setEnabled(False); self.refresh_midi_button.setEnabled(False)
        else:
            if self.input_thread: self.input_thread.stop(); self.input_thread.wait(); self.input_thread = None
            self.connect_button.setText("Connect"); self.connect_button.setChecked(False); self.midi_input_combo.setEnabled(True); self.piano_roll_widget.set_active_notes({}); self.refresh_midi_button.setEnabled(True)
    ...
    def set_controls_enabled(self, enabled: bool):
        is_file_selected = self.playlist_widget.currentItem() is not None
        is_input_connected = self.input_thread and self.input_thread.isRunning()
        ...
        self.show_tutorial_button.setEnabled(enabled)
        self.delay_spinbox.setEnabled(enabled)
        self.speed_slider.setEnabled(enabled)
        self.transpose_spinbox.setEnabled(enabled)
        self.refresh_midi_button.setEnabled(enabled and not is_input_connected)
        self.midi_input_combo.setEnabled(enabled and not is_input_connected)
